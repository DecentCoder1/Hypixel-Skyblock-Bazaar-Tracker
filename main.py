...
while True:
    ...
    bz2 = requests.get(r"https://api.hypixel.net/skyblock/bazaar")
    bz1 = dict(json.loads(bz2.text))
    bz3 = dict({
    ...
    bz_keys = list(bz3.keys())
    for x in bz_keys:
        quick_status.append(bz1["products"][x]["quick_status"])
        sell.append(round(bz1["products"][x]["quick_status"]["sellPrice"], 1))
        buy.append(round(bz1["products"][x]["quick_status"]["buyPrice"], 1))

    bz_list = {"Products": bz3.values(),  "Sell": sell, "Buy": buy}
    bz = pd.DataFrame(bz_list)
    bz.reset_index(drop=True, inplace=True)

    with open("bz.html", "w") as file:
        file.write(bz.to_html())
